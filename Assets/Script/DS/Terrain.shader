...
#include "AutoLight.cginc"
    ...
    float halfLambert = max(dot(worldLightDir, worldNormal), 0);

    fixed3 diffuse = _LightColor0.rgb * halfLambert * albedo;
    //fixed3 ambient = albedo*UNITY_LIGHTMODEL_AMBIENT.rgb;
    float3 halfDir = normalize(worldViewDir + worldLightDir);
    fixed3 specular = _LightColor0.rgb * albedo * max(dot(halfDir, worldNormal), 0);
