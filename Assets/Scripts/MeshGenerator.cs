...
public static class MeshGenerator
{
    public static MeshData GenerateTerrainMesh(float[,] heightMap, float heightMultiplier, AnimationCurve _heightCurve, int levelOfDetail)
    {
        ...
        float topLeftX = (meshSizeUnsimplified - 1) / -2f;  //to right
        float topLeftZ = (meshSizeUnsimplified - 1) / 2f;  //to down
        ...
        for (int y = 0; y < borderedSize; y += meshSimplificationIncrement)
        {
            for (int x = 0; x < borderedSize; x += meshSimplificationIncrement)
            {
                ...
                Vector3 vertexPosition = new Vector3(topLeftX + percent.x * meshSizeUnsimplified, height, topLeftZ - percent.y * meshSizeUnsimplified);  //former:meshData.vertices[vertexIndex]
...
public class MeshData
{
    ...
    {
        if (vertexIndex < 0)  //border vertice
        {
